...
* Perf: the fallback `WP_User_Query` that counts active sellers is now skipped when the count is hidden, so standalone Vendor Search blocks with the toggle off no longer run a full seller count on every render
