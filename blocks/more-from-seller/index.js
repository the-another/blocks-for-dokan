		...
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Settings', 'dokan-blocks' ) } initialOpen={ true }>
					...
					<SelectControl
						...
						value={ orderBy }
						options={ [
							{ label: __( 'Random', 'dokan-blocks' ), value: 'rand' },
							{ label: __( 'Date', 'dokan-blocks' ), value: 'date' },
							{ label: __( 'Title', 'dokan-blocks' ), value: 'title' },
							{ label: __( 'Price', 'dokan-blocks' ), value: 'price' },
							{ label: __( 'Popularity', 'dokan-blocks' ), value: 'popularity' },
						] }
			...
			<div { ...blockProps }>
				<Placeholder
					...
					instructions={ __( 'Displays products from the same vendor. Configure products count and layout in the sidebar.', 'dokan-blocks' ) }
				/>
			</div>
		</>
	);
