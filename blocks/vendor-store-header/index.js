		...
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Settings', 'dokan-blocks' ) } initialOpen={ true }>
					<SelectControl
						...
						value={ layout }
						options={ [
							{ label: __( 'Default', 'dokan-blocks' ), value: 'default' },
							{ label: __( 'Layout 2', 'dokan-blocks' ), value: 'layout2' },
							{ label: __( 'Layout 3', 'dokan-blocks' ), value: 'layout3' },
						] }
					...
					<ToggleControl
						...
						onChange={ ( value ) => setAttributes( { showStoreHours: value } ) }
					/>
				</PanelBody>
			</InspectorControls>
			...
			<div { ...blockProps }>
				<Placeholder
					...
					instructions={ __( 'Displays the vendor store header with banner, avatar, and contact info. Configure options in the sidebar.', 'dokan-blocks' ) }
				/>
			</div>
		</>
	);
